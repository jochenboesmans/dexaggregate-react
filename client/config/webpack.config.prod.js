...
module.exports = {
	mode: "production", // Don't attempt to continue if there are any errors.
	bail: true, // We generate sourcemaps in production. This is slow but gives good results.
	// You can exclude the *.map files from the build during deployment.
	devtool: shouldUseSourceMap ? "source-map" : false, // In production, we only want to load the app code.
	entry: [paths.appIndexJs], output: {
		// The build folder.
		path: paths.appBuild, // Generated JS file names (with nested folders).
		// There will be one main bundle, and one file per asynchronous chunk.
		// We don't currently advertise code splitting but Webpack supports it.
		filename: "static/js/[name].[chunkhash:8].js",
		chunkFilename: "static/js/[name].[chunkhash:8].chunk.js", // We inferred the "public path" (such as / or
		...
		minimizer: [new TerserPlugin({
			                             ...
			                             cache: true, sourceMap: shouldUseSourceMap,
		                             }), new OptimizeCSSAssetsPlugin({
			                                                             cssProcessorOptions: {
				                                                             parser: safePostCssParser,
				                                                             map: shouldUseSourceMap ? {
					                                                             // `inline: false` forces the sourcemap to be
					                                                             // output into a separate file
					                                                             inline: false, // `annotation: true` appends the
					                                                                            // sourceMappingURL to the end of
					                                                             // the css file, helping the browser find the
					                                                             // sourcemap
					                                                             annotation: true,
				                                                             } : false,
			                                                             },
		...
		strictExportPresence: true, rules: [// Disable require.ensure as it's not a standard language feature.
			...
			// It's important to do this before Babel processes the JS.
			{
				...
				oneOf: [// "url" loader works just like "file" loader but it also embeds
					...
					{
						test: /\.(js|mjs|jsx|ts|tsx)$/, include: paths.appSrc,
					...
					{
						test: cssRegex, exclude: cssModuleRegex, loader: getStyleLoaders({
							                                                                 importLoaders: 1,
							                                                                 sourceMap: shouldUseSourceMap,
						                                                                 }), // Don't consider CSS imports dead
						                                                                     // code even if the
					...
					{
						test: cssModuleRegex, loader: getStyleLoaders({
							                                              importLoaders: 1,
							                                              sourceMap: shouldUseSourceMap,
							                                              modules: true,
					...
					{
						test: sassRegex, exclude: sassModuleRegex, loader: getStyleLoaders({
							                                                                   importLoaders: 2,
							                                                                   sourceMap: shouldUseSourceMap,
						                                                                   },
						                                                                   "sass-loader"), // Don't consider CSS
						                                                                                   // imports dead code
						                                                                                   // even if the
						// containing package claims to have no side effects.
						// Remove this when webpack adds a warning or an error for this.
					...
					{
						test: sassModuleRegex, loader: getStyleLoaders({
							                                               importLoaders: 2,
							                                               sourceMap: shouldUseSourceMap,
							                                               modules: true,
							                                               getLocalIdent: getCSSModuleLocalIdent,
		...
		fs.existsSync(paths.appTsConfig) && new ForkTsCheckerWebpackPlugin({
		                                                                   ...
		                                                                   }),].filter(Boolean), // Some libraries import
	                                                                                           // Node modules but don't
	                                                                                           // use them in the
	                                                                                           // browser.
